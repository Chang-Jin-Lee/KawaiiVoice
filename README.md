...
**30세 남성 성우의 연기를 미소녀 여고생 음색으로.**

언리얼 엔진 탑다운 슈터 게임의 한국어 더빙 파이프라인 연구
...
[![Stars](https://img.shields.io/github/stars/Chang-Jin-Lee/KawaiiVoice?style=for-the-badge&logo=github&color=FFD43B)](https://github.com/Chang-Jin-Lee/KawaiiVoice/stargazers)
[![Last Commit](https://img.shields.io/github/last-commit/Chang-Jin-Lee/KawaiiVoice?style=for-the-badge&logo=git&logoColor=white&color=F05032)](https://github.com/Chang-Jin-Lee/KawaiiVoice/commits/main)
...
[📊 **조사 보고서**](./docs/01-research-report.md) · [⚙️ **파이프라인**](./docs/02-production-pipeline.md) · [🎙️ **음성 디렉션**](./docs/03-voice-direction.md) · [⚖️ **라이선스**](./docs/04-licensing.md) · [🎮 **언리얼**](./docs/05-unreal-integration.md)
...
> 별 수·라이선스·아카이브 여부는 GitHub API로 직접 조회했고, 라이선스 조항과 추론 파라미터는
> 실제 클론한 소스에서 판독했다. 조사 기준일 **2026-07-30**.
...
| | 용도 | 선택 | 라이선스 |
|:---:|---|---|:---:|
| 🏆 | **게임 최종 더빙** | [**Applio**](https://github.com/IAHispano/Applio) — RVC 프론트엔드 | `MIT` |
| ⚡ | **무학습 즉시 테스트** | [**Chatterbox**](https://github.com/resemble-ai/chatterbox) `ChatterboxVC` | `MIT` 코드+가중치 |
| 🎧 | **녹음 중 실시간 모니터링** | [**w-okada VCClient**](https://github.com/w-okada/voice-changer) | `MIT` |
| 🔁 | **대사 재합성이 허용될 때** | [**GPT-SoVITS**](https://github.com/RVC-Boss/GPT-SoVITS) — 60k★ | `MIT` |

</div>

**Applio를 고른 이유는 GUI가 아니다.** 바닐라 RVC에 없는 세 기능이 이 프로젝트의 문제를 정확히 겨냥한다 — `korean-hubert-base` 임베더, 포먼트 시프팅, 클립별 자동 피치. [→ 근거](#-주요-발견)
...
| **TTS** (GPT-SoVITS, IndexTTS2, CosyVoice) | 📝 텍스트 | **없음** — 대사를 새로 생성 | 🔁 대안 |
...
```mermaid
flowchart TD
    A["🎙️ 남성 성우 녹음<br/>감정·타이밍 그대로"] --> B{"음색 방향<br/>확정?"}
    B -- 아직 --> C["⚡ Chatterbox VC<br/>학습 0분 · MIT"]
    C --> B
    B -- 확정 --> D["👧 여성 화자 데이터셋<br/>20~40분 · 권리 명시 계약"]
    D --> E["🧠 Applio RVC 학습<br/>48kHz · korean-hubert-base · RefineGAN"]
    E --> F["🔀 Applio batch_infer<br/>자동 피치 255Hz · 포먼트 시프팅"]
    F --> G["🎚️ 디에서 → 라우드니스 정규화 → 무음 트림"]
    G --> H["🎮 언리얼<br/>Sound Concurrency · 더킹 · 바크 쿨다운"]
...
```bash
--embedder_model korean-hubert-base       # 한국어 음소 보존
...
<details open>
<summary><b>1. Applio를 골라야 하는 이유는 GUI가 아니다</b></summary>
...
| `--formant_shifting/qfrency/timbre` | 성별 인지의 상당 부분은 F0가 아니라 **성도 길이 = 포먼트 위치**에서 온다. 피치만 올리면 "빠르게 감은 남성 목소리"가 된다. |
| `--proposed_pitch` | 클립별 자동 피치 → 아래 3번 |

</details>

<details open>
<summary><b>2. Chatterbox가 Seed-VC 자리를 대체한다 — MIT로</b></summary>
...
코드와 **가중치 모두 MIT**라 상업 게임에 가장 안전하다. 단 소스에서 확인한 제약 3개:

- 모든 출력에 **Perth 신경 워터마크** 삽입 — MP3 압축·편집에도 잔존
- 참조음성 **앞 10초만** 사용 (`DEC_COND_LEN = 10 * S3GEN_SR`)
- S3 *의미 토큰* 경유라 **RVC보다 연기 보존력이 낮다** → 최종 축은 여전히 RVC

</details>

<details open>
<summary><b>3. 고정 피치 오프셋은 배치 작업에서 위험하다</b></summary>

<br>

성우 중심 피치는 테이크마다 달라진다(외침 vs 속삭임에서 한 옥타브 차이). 고정 `+12`는 외침에서 과보정돼 삑사리가 난다. `rvc/infer/pipeline.py`:
...
<details open>
<summary><b>4. ⚠️ fish-speech는 상업적 사용 금지다</b></summary>
...
<details>
<summary><b>5. Seed-VC는 아카이브됐고, Applio는 유지보수 모드다</b></summary>

<br>

- **`Plachtaa/seed-vc`** — 아카이브, 최종 푸시 `2025-04-20`. 저자의 저장소(`VALL-E-X`, `VITS-fast-fine-tuning`, `FAcodec`)가 **전부 아카이브**됐고 유지되는 실시간 포크는 존재하지 않는다.
- **`IAHispano/Applio`** — 자체 README 공지: *"Applio will no longer receive frequent updates ... focus mainly on security patches."* 이유는 **이미 안정·성숙 단계**라는 것이고, 필요한 기능은 모두 들어있으므로 본선 선택은 유효하다. 다만 **새 기능을 기대하고 의존해선 안 된다.**
...
`ARCH` = GitHub 아카이브(읽기 전용). **전체 30개 표는 [조사 보고서](./docs/01-research-report.md)에 있다.**
...
| [so-vits-svc](https://github.com/svc-develop-team/so-vits-svc) | 28,152 | `AGPL-3.0` | ⛔ | 2023-11-11 | SVC |
...
| [IndexTTS2](https://github.com/index-tts/index-tts) | 22,256 | ⚠️ bilibili 독자 | | 2026-07-14 | TTS (감정 제어) |
| **[VCClient](https://github.com/w-okada/voice-changer)** | **20,694** | **`MIT`** | | 2026-03-21 | **실시간 VC** |
| [seed-vc](https://github.com/Plachtaa/seed-vc) | 3,893 | `GPL-3.0` | ⛔ | 2025-04-20 | 무학습 VC |
...
검증된 파라미터가 기본값으로 들어있다. 상세는 [파이프라인 문서](./docs/02-production-pipeline.md).
...
| | 문서 | 내용 |
|:---:|---|---|
| 📊 | [**01 · 조사 보고서**](./docs/01-research-report.md) | 후보 30종 검증 비교, ChatGPT 답변 교차검증, 정정 6건 |
| ⚙️ | [**02 · 제작 파이프라인**](./docs/02-production-pipeline.md) | 소스 검증된 파라미터, 학습 설정, 파일럿 A/B 프로토콜 |
| 🎙️ | [**03 · 음성 디렉션**](./docs/03-voice-direction.md) | 성우 연기 지시, 짧은 전투 바크가 가장 어려운 이유 |
| ⚖️ | [**04 · 라이선스 체크리스트**](./docs/04-licensing.md) | 상업 출시 권리 검토, 음성 데이터 계약 조항 |
| 🎮 | [**05 · 언리얼 통합**](./docs/05-unreal-integration.md) | 동시성 제어, 더킹, 탑다운 공간화, 검수 체크리스트 |
...
<table>
<tr><th align="left">🚫 하지 말 것</th><th align="left">✅ 할 것</th></tr>
<tr valign="top"><td>

- 인터넷 배포 **"애니 캐릭터 / 성우 RVC 모델"** — 배포자에게 재라이선스 권한이 없는 경우가 대부분
- 애니·게임·유튜브에서 추출한 음성으로 학습
- 실존 성우·연예인 목소리 모방

</td><td>

- **직접 섭외한 화자**를 녹음
- 계약서에 **AI 학습 / 음성 변환 합성 / 상업적 배포 / 2차 활용 범위 / 철회권 처리** 명시
- 남성 성우에게도 본인 음성이 변환·사용됨을 고지하고 동의 취득

</td></tr>
</table>

전체 체크리스트 → [**라이선스 문서**](./docs/04-licensing.md)
...
> 음질 관련 서술은 소스 코드·라이선스에서 확인한 사실과 아키텍처적 추론이다.
> **최종 음색 판단은 [파일럿 A/B 프로토콜](./docs/02-production-pipeline.md)**(대표 20개 대사 × 5개 축)로
...
이 조사는 다음 프로젝트들의 **소스와 라이선스를 직접 판독**해 작성했다.
...
여기서 다루는 외부 도구들의 라이선스는 각기 다르며 **일부는 상업적 사용이 제한된다** → [라이선스 체크리스트](./docs/04-licensing.md)

<div align="center">
<br>
<sub>조사 기준일 2026-07-30 · 별 수와 라이선스는 변한다. <a href="./scripts/audit_repos.sh"><code>audit_repos.sh</code></a>로 재검증할 것.</sub>
</div>
